...
// Hàm main
int main() {
    cout << "\n***** BAI 8a: THUAT TOAN VUONG HAO (OOP) *****\n" << endl;
    cout << "Bai toan: Cho {p->q, q->r}. Chung minh: {p->r}" << endl;
    
    // Tạo hệ thống suy diễn
    InferenceEngine* engine = new InferenceEngine();
    
    // Thêm các luật
    engine->addRule(new Rule("R1", {"p"}, "q"));  // p → q
    engine->addRule(new Rule("R2", {"q"}, "r"));  // q → r
    
    // Thiết lập cơ sở sự kiện ban đầu (có p)
    FactBase* factBase1 = new FactBase({"p"});
    ...
    // Thử trường hợp không có sự kiện ban đầu
    cout << "\n***** THỬ TRƯỜNG HỢP KHÔNG CÓ 'p' BAN ĐẦU *****\n" << endl;
    
    // Reset hệ thống
    engine->reset();
    
    // Thiết lập cơ sở sự kiện mới (rỗng)
